"""
scrapers/flipkart/normalizer.py
===============================
Canonical product normalizer for Flipkart items.
"""
...
logger = logging.getLogger("smartbuy.scrapers.flipkart.normalizer")


def normalize_flipkart_product(raw: Dict[str, Any], query: str = "") -> Optional[Dict[str, Any]]:
    """Normalize raw Flipkart product dictionary into SmartBuy authoritative schema."""
    ...
    product_id = raw.get('product_id') or f"fk_{abs(hash(url)) % 1000000}"
    ...
    built_data.update({
        ...
        "source": raw.get("source") or "flipkart",
    })

    product = validate_and_build_product(built_data, platform="flipkart")
