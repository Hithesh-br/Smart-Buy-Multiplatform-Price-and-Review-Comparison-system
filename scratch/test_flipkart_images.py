...
page.goto("https://www.flipkart.com/search?q=chia+seeds", wait_until="domcontentloaded", timeout=18000)

soup = bs4.BeautifulSoup(page.content(), "html.parser")
cards = soup.select('div[data-id], div.slAVV4, div.jIjQ8S, div._1AtVbE')
print(f"Total cards: {len(cards)}")

from scrapers.flipkart.parser import parse_flipkart_card
valid_parsed = 0
for i, c in enumerate(cards[:5]):
    item = parse_flipkart_card(c)
    if item:
        valid_parsed += 1
        print(f"Card {i+1}:")
